...
Mythili was tasked with developing a program that collects information from users. The program should prompt the user to enter their 
employee ID (integer), salary (decimal), and department code (character). Once the user provides the required information, the program 
should display the entered values for verification using the 'cin' statement and display the output using the 'cout' statement.
...
int main() {
    ...
    double salary;
    char departmentCode;
    cin >> employeeID >> salary >> departmentCode;
